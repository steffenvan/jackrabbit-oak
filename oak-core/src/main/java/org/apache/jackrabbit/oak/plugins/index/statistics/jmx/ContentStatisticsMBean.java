...
public interface ContentStatisticsMBean {
    ...
    Optional<EstimationResult> getSinglePropertyEstimation(
            @Description("The property name (e.g jcr:primaryType)")
            @Name("name") String propertyName);
     ...
     */
    @Description(
            "Returns the statistical information of all properties in the repository. This includes the estimated count, cardinality, average value length, top K most frequent values and more.")
     ...
     *
     * @param name - the property for which we want the top K values for
